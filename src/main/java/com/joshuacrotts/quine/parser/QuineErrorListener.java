 ...
 * Simply call QuineErrorListener.getErrorIterator() and
 * QuineErrorListener.getWarningIterator() to get an Iterator object for them.
 *
 * @author Joshua Crotts
 * @modified 2/20/2021
 */
public class QuineErrorListener extends BaseErrorListener {
    ...
    public static void syntaxError(ParserRuleContext ctx, String errorMsg) {
        QuineErrorListener.gotParserError = true;
        int lineNo = -1;
        int colNo = -1;
        ...
        QuineErrorListener.errors.add(new Message(errorMsg, colNo));
    }
    ...
    public static void proofVerifierError(String _errorMsg) {
        QuineErrorListener.proofVerifierErrors.add(new Message(_errorMsg, 0));
    }
    ...
    public static void reset() {
        QuineErrorListener.errors.clear();
        QuineErrorListener.proofVerifierErrors.clear();
        QuineErrorListener.gotParserError = false;
        QuineErrorListener.gotLexerError = false;
    }
    ...
    public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line, int col, String errorMsg,
        ...
        QuineErrorListener.errors.add(new Message(errorMsg, col + 1));
    }
    ...
    private boolean isBinaryOpToken(int _tokId) {
        return _tokId == QuineLexer.AND || _tokId == QuineLexer.OR
                || _tokId == QuineLexer.IMP || _tokId == QuineLexer.BICOND || _tokId == QuineLexer.XOR;
    }

    private boolean isWhitespaceToken(int _tokId) {
        return _tokId == QuineLexer.WHITESPACE;
    }

    private boolean isOpenParenthesisToken(int _tokId) {
        return _tokId == QuineLexer.OPEN_PAREN;
    }

    private boolean isCloseParenthesisToken(int _tokId) {
        return _tokId == QuineLexer.CLOSE_PAREN;
    }
